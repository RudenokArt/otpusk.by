...
<?if(!empty($arResult["ITEMS"])):?>	
	...
	?>
		...
		<div class="package-list-item clearfix">
			...
			<?endif?>
			<?= printTourLabel($p)?>
			<?if(!empty($i["PROPERTIES"]["OFFER_IN_PROCESS"]["VALUE"])):?>
			...
			<div class="content">
				<span style="display: inline-block; font-size: 17px; margin: 18px 0 10px;"><a style="color: black;" href="<?= $i['DETAIL_PAGE_URL']?>"><?= $i["NAME"]?></a></span>
				<div class="row gap-10">
					<div class="col-sm-12 col-md-9">
						...
						<ul class="list-info">
                            ...
                            <?if(!empty($p["TOUR_IN_ARCHIVE"]["DISPLAY_VALUE"]) && ($p["TOUR_IN_ARCHIVE"]["DISPLAY_VALUE"] == "Y")):?>
                                <li><span class="archive-tour">Тур добавлен в архив, возможно он скоро поступит в продажу</span></li>
                            <?endif?>
						</ul>
						
					</div>

					<? $cur_currency = CIBlockElement::GetByID($p["CURRENCY"]["VALUE"])->GetNext(); ?>
					...
					<div class="col-sm-12 col-md-3 text-right text-left-sm"><center>
						...
						<div class="price" style="line-height:16px; border:1px solid"><span>от</span><br>
								<?=$p["PRICE"]["VALUE"] . " " . $cur_currency["NAME"] ?><br>
