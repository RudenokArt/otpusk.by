...
if(count($arResult['hotels']) > 3)
{
	...
	$arResult['all_hotels'] = makeFilterLink("TOWN", Set::HOTELS_IBLOCK_ID, $arResult['ID'], '', $arParams["FILTER_NAME"]);
...
{
	// ссылка на фильтр отелей по стране
	$arResult['all_sanatorii'] = makeFilterLink("TOWN", Set::HOTELS_IBLOCK_ID, $arResult['ID'], Set::SANATORII_SECTION_ID, $arParams["FILTER_NAME"]);
...
if(count($arResult['offers']) > 3)
{
	...
	$arResult['all_offers'] = makeFilterLink("TOWN", Set::SPECIAL_OFF_IBLOCK_ID, $arResult['ID'],'', $arParams["FILTER_NAME"]);
...
{
	// ссылка на фильтр туров по стране
	$arResult['all_sights'] = makeFilterLink("TOWN", Set::SIGHTS_IBLOCK_ID, $arResult['ID'], '', $arParams["FILTER_NAME"]);
