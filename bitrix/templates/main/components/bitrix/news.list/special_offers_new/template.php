                ...
                <div class="section-title">
                    ...
                    <div class="sorting-middle-holder">
                        <ul class="sort-by s-id-<?= $s_id ?>">
                            <? if (count($arResult["TYPE_TOURS"]) > 1): ?>
                                <? foreach ($arResult["TYPE_TOURS"] as $id => $n): ?>
                                    <li><a data-cid="<?= $id ?>" class="sof-<?= $s_id ?>"
                                           data-group=".gc-<?= $s_id ?>-<?= $id ?>" href="#"><?= $n ?></a></li>
                                <? endforeach ?>
                            <? endif ?>
        ...
        <div class="GridLex-gap-30-wrappper package-grid-item-wrapper">
            ...
            <div id="grid-<?= $s_id ?>" class="GridLex-grid-noGutter-equalHeight  ajax-<?= $s_id ?>">
                <? foreach ($arResult["ITEMS"] as $i):
                    ...
                    $d_g = implode(" ", $d_g);
                    ?>
                    <div id="<?= $this->GetEditAreaId($i['ID']); ?>"
                         class="filtering-item GridLex-col-3_sm-6_xs-12 mb-30 <?= $d_g ?>">
                        <div class="package-grid-item">
                            <a href="<?= $i["DETAIL_PAGE_URL"] ?>">
                                <div class="image">
                                    ...
                                    <? if ($i['PROPERTIES']['DAYS']['VALUE']):?>
                                        <div class="absolute-in-image">
                                            <div class="duration"><span><?= w($i['PROPERTIES']['DAYS']['VALUE']) ?>
                                                    / <?= w($i['PROPERTIES']['DAYS']['VALUE'], 2) ?></span></div>
                                        </div>
                                    <? endif ?>
                                </div>
                                <div class="content clearfix">
                                    ...
                                    <a href="<?= $i["DETAIL_PAGE_URL"] ?>" class="btn_blue_p6" style="float: right;">Заказать
                                        тур</a>
                                    <? if ((int)$i["DISPLAY_PROPERTIES"]["PRICE"]["VALUE"] > 0):?>
										...
										<? /*if($i["DISPLAY_PROPERTIES"]["CURRENCY"]['VALUE'] != 62):*/
                                                ?>
										<span style="font-weight:normal; font-size:12px;"> (<? /*= $i["DISPLAY_PROPERTIES"]["PRICE"]["VALUE"] .' '. strip_tags($i["DISPLAY_PROPERTIES"]["CURRENCY"]['DISPLAY_VALUE'])*/
                                                ?>)</span>
										--><? /*endif*/
                                                ?><br>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $(document).on("click", ".sof-<?= $s_id?>", function (e) {
            ...
            $.ajax({
                url: '/ajax/special_offers_new.php',
                method: 'POST',
                data: params,
