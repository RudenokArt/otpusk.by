<?
	...
	$APPLICATION->SetTitle("Каталог аэропортов. ".$name_cn. " - список городов");
		...
		require($_SERVER["DOCUMENT_ROOT"]."/bitrix/header.php");
			...
			$arFilter = Array("IBLOCK_ID"=>14,"PROPERTY_ISO"=> $iso_cn);
	...
	foreach($cities_lib as $key=>$value)
	{
		...
		if($current_letter != strtoupper(substr($value[ct_name],0,1)))
		{
			...
			echo "<span class='catalog_letter'>$current_letter</span>\n";
		}

		echo "<hr style='margin:5px 0px'><h3>".$value[ct_name]."</h3>\n";
		...
		{
			$ap_slug = slug($ap_en);
			echo "<li><a href='$ap_slug/'>$ap_ru ($ap_slug)</a></li>\n";
		}
