...
<div class="col-sm-8 col-md-9" role="main">
	...
	<div class="payment-container">
		...
		<form class="booking-form" autocomplete="off" action="<?= POST_FORM_ACTION_URI?>" method="post">
			<?= bitrix_sessid_post()?>
				...
				<div class="payment-content">
					...
					<div class="form-horizontal">
						<div class="mb-0 form-group gap-20">
							...
							<div class="col-sm-5 col-md-4">
								<input pattern="^\+?[0-9,\s]{0,}$" placeholder="+375291111111" required="" type="text" value="<?if ($arResult['POST']['phone'] != ""):?><?=$arResult['POST']['phone'];?><?endif?>" name="phone" class="form-control">
							</div>
						</div>
	...
	<h4 class="heading mt-0 text-primary uppercase">Ваш заказ</h4>
		...
		<?foreach ($arResult['CART'] as $k=>$arItem): ?>
			<li class="pos-rel">
				...
				Курорт: <?= $arItem['CITY']?><br>
				Заезд: <?= $arItem['CHECK_IN']?><br>
				Выезд: <?= $arItem['CHECK_OUT']?><br>
				Тип номера: <?= $arItem['ROOM_TYPE']?><br>
				<?if($arItem['ROOM_CAT'] != "Not defined"):?>Номер: <?= $arItem['ROOM_CAT']?><br><?endif?>
				Питание: <?= $arItem['MEAL']?><br>
				Туристов: <?= $arItem['PEOPLE']?><br>
				Цена: <?= $arItem['PRICE']?>  <?if($arItem["DEFAULT_RATE"] != $arResult["CURRENCY"]):?>(<?=$arItem["DEFAULT_PRICE_RATE"]?>)<?endif?><br>
				</p>
			...
			<li class="total-price">
				...
				<div class="row gap-10">
					...
					<div class="col-xs-6 col-sm-6 text-right">
						<span class="block font20 font600 mb-5"><?= $arResult['TOTAL_PRICE']?> <?= $arResult['CURRENT_CURRENCY']?></span>
